...
abstract class SinglePointGen extends Gen {
	...
	protected byte changeableRGB;// BYTE Containing the color set to change
									// r(1)g(2)b(3).
	protected float harsh;// FLOAT Containing the multiplier for how much
							// currentBaseColor's valRGB will change.

	protected short[] calcIntervals(short input) {
		short[] out = new short[currentSteps];

		for (int i = 0; i < currentSteps; i++)
			out[i] = (short) (input - ((input * harsh) / (currentSteps - 1) * i));
	...
	protected short[] calcIntervalsR(short input) {
		...
		for (int i = 0; i < currentSteps; i++)
			out[i] = (short) (input + ((mod * harsh) / (currentSteps - 1) * i));
